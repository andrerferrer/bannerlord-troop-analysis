...
def _specs() -> tuple[FindingSpec, ...]:
    ...
    return (
        ...
        FindingSpec("role_scores_v1", v1, False, False, False, False, "IRRELEVANT_DRIVER_INCLUDED", "ranged_raw", "Weapon speed, accuracy, missile speed, and a capped stack bonus enter ranged output."),
        ...
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "CONTEXT_UNDECLARED", "candidate declaration", "The candidate is defensive but does not select field, siege attack, or siege defense."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "ATTACK_MODE_UNDECLARED", "candidate declaration", "Melee counterpressure is read without a declared attack mode."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "MOUNT_STATE_UNDECLARED", "defensive_lane", "Mounted/unmounted lanes replace an explicit context and mount-state declaration."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "QUESTION_MIXED", "protection_score_v2 and defensive_utility_score_v2", "Protection and utility answer separate questions and utility blends mobility/melee skill."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "IRRELEVANT_DRIVER_INCLUDED", "shield_hp_component_v2", "Shield endurance is treated as physical protection rather than a separate blocking question."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "IRRELEVANT_DRIVER_INCLUDED", "shield_armor_component_v2", "Shield armor is included without an explicit shield-endurance question."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "IRRELEVANT_DRIVER_INCLUDED", "harness_component_v2 and mount_health_component_v2", "Mounted durability enters a context-undeclared defense candidate."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "IRRELEVANT_DRIVER_INCLUDED", "mobility_component_v2", "Horse speed/maneuver or Athletics enter defensive utility."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "IRRELEVANT_DRIVER_INCLUDED", "counterpressure_component_v2", "Maximum melee skill enters defensive utility."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "MISSING_VALUE_ZERO_FILLED", "number(value, default=0.0)", "Missing/non-finite values are converted to a numeric default in historical feature helpers."),
        FindingSpec("defensive_role_scores_v2_candidate", v2, False, True, False, False, "MOUNTED_INPUT_NON_APPLICABLE", "cavalry protection/utility lanes", "Mount and harness inputs cannot answer siege-defense defense after dismounting."),
        ...
        FindingSpec("v7.3", v73, False, False, False, False, "IRRELEVANT_DRIVER_INCLUDED", "skill, damage type, reliability, and defense factors", "The base context-first attack candidate excludes these undeclared secondary drivers."),
...
def _protected_paths(repo: Path) -> Iterable[tuple[Path, str]]:
    ...
    for root, classification in roots:
        if not root.is_dir():
            raise AuditError(f"protected root missing: {root.relative_to(repo).as_posix()}")
        for path in root.rglob("*"):
            if path.is_symlink():
                raise AuditError(f"protected path is a symlink: {path.relative_to(repo).as_posix()}")
            if path.is_file():
                yield path, classification

    theoretical_root = repo / "analysis/theoretical"
    export_roots = sorted(theoretical_root.glob("*/export_20260731_150800"))
    if not export_roots:
        raise AuditError("no historical export_20260731_150800 directories found")
    for root in export_roots:
        for path in root.rglob("*"):
            if path.is_symlink():
                raise AuditError(f"protected path is a symlink: {path.relative_to(repo).as_posix()}")
            if path.is_file():
                yield path, "historical_output"

    for relative in AUDITED_SOURCES:
        path = repo / relative
        if not path.is_file():
            raise AuditError(f"audited source missing: {relative}")
        yield path, "audited_source"
...
def write_audit(repo: Path, report_output: Path, baseline_output: Path) -> None:
    repo = repo.resolve()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        write_audit(repo, report, baseline)
