...
The fixture also includes source XML with two `<Weapon>` components whose scalar audit representation would lose one row, a verified raw-manifest hash, duplicate/missing tooltip observations, and an exact reconstructed-output hash. This proves the normalizer and receipt producer rather than assuming their outputs.
...
Given cavalry and infantry have equal worn armor but different shields, weapons, Riding, mounts, harnesses, and charge, when `siege_defense + defense` runs, both effective mount states are dismounted and their defense component/rank ties. A mutation test that changes every mounted field must leave scoring components, rankable rows, and ranks unchanged. Input provenance, input hashes, metadata, and outer package hashes are expected to change because the source bytes changed.

### F2 — Finite field ranged attack

Given a `Bow`, two `Arrow` stacks, and an unrelated `Bolt` stack, when `field + attack + ranged + finite` runs, output equals `weapon damage * sum(Arrow stack_amount)`. The bolt is published as incompatible or ignored with provenance, and mutating projectile damage cannot change candidate-v1 output.
