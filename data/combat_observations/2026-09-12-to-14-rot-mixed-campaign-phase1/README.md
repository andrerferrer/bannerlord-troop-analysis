...
- `screenshots_manifest.csv` — authoritative source-to-battle manifest using the repository schema.
...
- `phase1_checkpoint.json` / `batch_state.json` — workflow checkpoint and current Phase 1 bundle identity.
- `bundle/README.md` — archive reconstruction and deterministic rebuild verification.
- `bundle/rot_mixed_campaign_phase1.members.sha256` — hash for every archive member.
- `handoff/ANALYSIS_PROMPT.md` — Phase 2 instructions for a separate downstream agent.
- `handoff/ANALYSIS_TASK_V1.json` — machine-readable Phase 2 task metadata.
